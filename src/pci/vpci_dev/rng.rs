...
use crate::pci::pci_struct::{LockedVirtualPciConfigSpace, PciConfigSpace};
...
impl VpciDeviceHandler for VirtioRngHandler {
    ...
    fn read_cfg(&self, space: LockedVirtualPciConfigSpace, offset: PciConfigAddress, size: usize) -> HvResult<PciConfigAccessStatus> {
        // info!("virt pci standard read_cfg, offset {:#x}, size {:#x}", offset, size);
        let space_guard = space.read();
    ...
    fn write_cfg(&self, space: LockedVirtualPciConfigSpace, offset: PciConfigAddress, size: usize, value: usize) -> HvResult<PciConfigAccessStatus> {
